...
function sayAtColumns (source, strings, numCols) {
	...
	if ((col) % numCols !== 0) Broadcast.sayAt(source, '');
}
...
module.exports = {
	...
	command: (state) => (args, player) => {
		...
		Broadcast.sayAt(player, "<bold><white>" + '='.repeat(80) + "</bold></white>");

		let commands = [];
		for (let [ name, command ] of state.CommandManager.commands) {
			if (player.role >= command.requiredRole) commands.push(name);
		}
		
		Broadcast.sayAt(player, commands.length + ' commands');
		commands.sort()
